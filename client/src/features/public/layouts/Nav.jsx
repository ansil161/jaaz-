...
import { nav } from '@/features/public/data/site'
    ...
    <div>
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[0.1] bg-ink">
        ...
        <div className="shell grid grid-cols-[1fr_auto_1fr] items-center gap-4 py-4 sm:py-5">
          {/* THE MARK IS THE SUPPLIED LOCKUP, NOT A TYPESET WORD.
              Two decisions come with it:

              1. The REVERSE artwork, not the original file. The bar is
                 opaque `--color-ink` (see above), and the delivered
                 lockup's inks are a #4E5659 slate and a #234A67 navy —
                 both effectively invisible on black. `jaz-mark-reverse`
                 is the same drawing with each ink lifted to its light
                 counterpart at the same hue, so the mark keeps its two
                 tones instead of being flattened to a white knockout by
                 a CSS filter.
              2. No tagline. The full lockup stacks "home theater
                 systems" under the logotype; at the 36–40px this track
                 allows that line renders about three pixels tall and
                 reads as grit. `jaz-mark-reverse` is cropped to BRIO +
                 the logotype — the full lockup, tagline included, runs
                 in the footer where it has the height to be read.

              `width`/`height` are the intrinsic pixels so the bar
              reserves the box before the PNG lands; the height is set
              in CSS and the width follows. */}
          <Link to="/" className="focus-ring justify-self-start" aria-label="JAZ — home">
            <img
